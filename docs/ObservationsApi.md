...
> EdGraphHttpAggregatorsTenantApiServicesObservationsGetCampusesWithEvalueesResponse getPaginatedCampusesWithEvaluees()
...
const { status, data } = await apiInstance.getPaginatedCampusesWithEvaluees(
    tenantId,
    pageSize,
    pageIndex,
    orderBy
